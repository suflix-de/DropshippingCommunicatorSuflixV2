...
class ExportController extends Controller
{
    public function sendOrder(Request $request, Response $response, int $orderId): \Symfony\Component\HttpFoundation\Response
    {
        try {
            ...
            // SwiftMailer via statische newInstance() Factory-Methoden (kein "new" nötig)
            ...
            // TXT-Anhang
            $message->attach(
                \Swift_Attachment::newInstance($txtContent, $txtFilename, 'text/plain')
            );
            ...
            foreach (($orderArray['documents'] ?? []) as $doc) {
                if (($doc['type'] ?? '') === 'deliveryNote' && !empty($doc['content'])) {
                    ...
                    $message->attach(
                        \Swift_Attachment::newInstance(base64_decode($doc['content']), $pdfName, 'application/pdf')
                    );
                    break;
                }
            }
