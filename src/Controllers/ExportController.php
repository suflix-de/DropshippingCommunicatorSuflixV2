...
class ExportController extends Controller
{
    public function sendOrder(Request $request, Response $response, int $orderId): \Symfony\Component\HttpFoundation\Response
    {
        try {
            ...
            $order      = $authHelper->processUnguarded(fn() => $orderRepo->findOrderById($orderId));
            ...
            // ── Dokumente laden ──────────────────────────────────────────────
            $documents = [];
            $docMethods = ['listOrderDocuments', 'findDocuments', 'findByOrderId', 'getDocumentsByOrderId'];
            foreach ($docMethods as $method) {
                try {
                    ...
                    $docs    = $authHelper->processUnguarded(fn() => $docRepo->$method($orderId));
                    if (!empty($docs)) {
                        $documents = is_array($docs) ? $docs : iterator_to_array($docs);
                        break;
                    }
                } catch (\Throwable $e) {}
            }
