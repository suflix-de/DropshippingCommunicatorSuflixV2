...
class ExportController extends Controller
{
    public function sendOrder(Request $request, Response $response, int $orderId): \Symfony\Component\HttpFoundation\Response
    {
        try {
            ...
            // ── SMTP altConfig – umgeht PlentyONE Mail-Service komplett ──────
            ...
            // Senden: mit eigenem SMTP (altConfig) → PlentyONE Mail-Service wird umgangen
            $mailer->sendFromMime($mime, $recipients, $altConfig, [], $bcc);

            return $response->json([
                'success'    => true,
                'message'    => 'E-Mail mit Anhang versendet!',
                'recipients' => $recipients,
                'subject'    => $subject,
                'smtp'       => empty($altConfig) ? 'PlentyONE Standard' : $smtpHost,
            ]);
